...
import { fetchHomePostDetail } from '../../../Features/homePostDetail/homePostDetailSlice';
...
const HomeDisplayContent = () => {
	const [data, setData] = useState(18);
	let location = useLocation();
    const dispatch = useDispatch();
    const pathname = location.pathname;
    const path = pathname.slice(1);
    const { homePostDetail,isLoading } = useSelector((state) => state.homePostDetail);
		const { posts } = useSelector((state) => state.posts);
		const {feature_post, feature_post_2,latest_post} = useSelector(
			(state) => state.homePost
	);
    console.log(homePostDetail);
    useEffect(() => {
        dispatch(fetchHomePostDetail(path));
    }, [path]);
	...
	return (
		...
		<div>
						...
						<div class="row">
								<div class="col-xxl-9 col-xl-9 col-md-9 col-sm-12 col-12">
										<div>
												<div class="loading-title1">
														<a class="d-block" href="">
																<p class="cards-title2-1 pt-xxl-2 pt-xl-2 pt-md-2 pt-3 pb-3">
																		{homePostDetail?.category?.name_bn}
																</p>
														</a>
												</div>
												...
												<h1 class="cards-title1-1">
														{homePostDetail?.title_bn}
												</h1>
												<p class="cards-content-text-time">
														{homePostDetail?.editor?.name}
												</p>
												<div class="d-flex flex-xxl-row flex-lg-row flex-md-row flex-sm-column flex-column justify-content-between mb-3">
														<p class="cards-content-text-time d-flex gap-2">
																{`প্রকাশ:`}
																<MomentConverter
																		date={homePostDetail?.published_at}
																		formate={"LL, HH:mm"}
																/>
														</p>
														<div>
														...
														<div class="col-xxl-9 col-xl-9 col-md-9 col-sm-12 col-12 m-auto">
																...
																<div>
																		<p
																				...
																				class="cards-title-4 overflow-hidden"
																		>
																				{homePostDetail?.content_bn}
																		</p>
																</div>
														</div>
												</div>
								...
								<div class="col-xxl-3 col-xl-3 col-md-3 col-sm-12 col-12">
										...
										<div class="row">
												...
												{feature_post?.slice(0, 1).map((description) => {
														return (
												...
												{feature_post?.slice(2, 6).map((description) => {
														return (
																<div class="col-12">
																		<div class="cards">
																				<Link
				...
				<div class="container">
						<div class="row">
								{latest_post?.slice(0, 1).map((description) => {
										return (
								...
								{latest_post?.slice(7, 11).map((description) => {
										return (
...
export default HomeDisplayContent;